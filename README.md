# Part 1. BDES链系统互操作API -- JS

Javascript API，用于使用BDES- RPC API与基于BDES的区块链集成。
...
```html
 <script type="text/javascript" src="bdes.min.js"></script>
 ```
...
```JavaScript
var bdes=new BDES({
            default_ws_node:"ws://113.105.131.113:9901",//XXXXXXXXX” //节点rpc地址,选填。如果没有指定此项则会自动连接ws_node_list中速度最快的节点
            ws_node_list:[{url:"ws://113.105.131.113:9901",name:"东莞节点"}]//API服务器节点列表，必填
            faucet_url:"http://113.105.131.113:9941", //注册入口
            networks:[{
                core_asset:"BDES",//核心资产符号
                chain_id:"e7b438321fb03f82b27ef8693abf5fe30509144bb9ebdfdb775575fc4424c172"//链id   
            }], 
...
```
#### 8.非查询类接口若涉及关联ID业务(如创建数据资产产生的ID)返回的数据中将包含data对象
//示例
```js
data:{
...
| 131 | Parameter 'assetType' can not be empty | 数据类型名称不能为空 | creatAssetType | 
...
| 141 | Please check the data in parameter NHAssets | 请检查参数NHAssets中的数据 | creatNHAsset批量创建数据 | 
| 142 | Please check the data type of parameter NHAssets | 请检查参数NHAssets的数据类型 | creatNHAsset批量创建数据 | 
| 144 | Your current batch creation / deletion / transfer number is X , and batch operations can not exceed X | 您当前批量 创建/删除/转移 数据资产数量为 X ，批量操作数量不能超过 X | creatNHAsset、deleteNHAsset、transferNHAsset | 
...
| 164 | assetTypes do not exist | 数据类型不存在 | queryAssetTypes | 
...
### 创建数据类型  
方法：creatAssetType  
功能：创建支持的数据资产数据类型，向区块链系统注册当前账号支持的数据资产数据类型  
参数：  
	assetType：数据类型名称，区分大小写;  
  
### 创造数据资产  
方法：creatNHAsset
功能：创建一个唯一的数据资产，具有唯一性。
参数：  
	assetId：当前数据资产交易时，适用的资产ID；  
	assetType：数据类型；  
	baseDescribe：当前数据资产的具体内容描述，由制造者定义;  
	ownerAccount：指定数据资产拥有者(数据资产归属权账户，默认为数据资产创建者)  
	NHAssetsCount (Number)：创建数据资产的数量，默认值为1，只有type为0即创建同一种数据资产生效  
	type：创建数据资产方式的类型，默认值为0。值为0时默认创建同一种数据资产，1是创建不同数据资产。  
	NHAssets(Array)示例:   
```JavaScript  
	[{  
	 "assetId": "X.X.X",  
...
### 删除数据资产  
方法：deleteNHAsset 
功能：删除整条数据资产数据记录，通常在商品销毁时使用（仅能由用户自己授权处理自己想要销毁的数据）。  
参数：  
	NHAssetIds (Array)：数据资产实例的唯一标识ID;示例：\[X.X.X, X.X.X\]  
  
### 转移数据资产  
方法：transferNHAsset 
功能：用户可以将自己的数据资产转移到另外一个用户  
参数：  
	toAccount：转移数据资产的目标用户名  
	NHAssetIds（Array）：多个数据资产id组成的数组，示例：\[X.X.X, X.X.X\]  
  
### 提议关联数据类型  
方法：proposeRelateAssetType  
功能：提议关联到某一个数据类型，需要该数据类型的创建人审批  
参数：  
	assetType：需要关联的数据类型名  
  
### 批准关联数据类型的提议  
方法：approvalProposal  
功能：批准其他用户关联自己的数据类型的提议  
参数：  
	proposalId：提议ID  
...
功能：卖出数据资产（在交易前可调用queryAccountNHAssets函数，列举用户数据资产，以便用户选着卖出）  
参数：  
	otcAccount：OTC交易平台账户，用于收取挂单费用；（OTC平台填写）  
	orderFee：挂单费用，用户向OTC平台账户支付的挂单费用；（OTC平台填写）  
	NHAssetId：数据资产实例的唯一标识ID; （用户填写）  
	price：商品挂单价格；（用户填写）  
...
### 购买订单数据资产  
方法：fillNHAssetOrder  
功能：买入数据资产，支付购买数据资产的代币费用，同时修改用户拥有的商品数据。该操作是一个多步合成的原子操作，在支付费用的同时完成用户账户数据资产数据的更新，如果支付动作或账户商品数据更新动作中某一个动作不被主链区块认可，则整个交易将被回滚，避免异常交易。  
参数：  
	orderId：订单ID  
	callback：回调函数  
...
参数：  
	...
	assetTypes(string)：版本名称或版本id筛选条件  
...
功能：读取当前用户账户下所有可在对应数据类型中使用的数据资产  
参数：  
	account：账户名或账户id  
	assetTypes (array)：数据类型名集合  
	page：页数  
...
参数：  
	...
	functionName：合约里的函数名称，my_nht_describe_change （修改数据资产属性）  
...
### 交易备注解密   
方法：decodeMemo  
功能：无回调，直接返回结果，结果是一个对象，对象中包含备注文本text。该方法传参是直接传入，非包裹式options对象传参。 示例：bdes.decodeMemo(raw_data.memo) ,其中raw_data为交易原始数据。
...
| creat_asset_type | creatAssetType |   
| propose_relate_asset_type | proposeRelateAssetType |   
